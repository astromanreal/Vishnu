...
import { MapPin, Landmark, Building, Users, Globe, Link as LinkIcon } from 'lucide-react';
import { getTempleBySlug, type Temple } from '@/data/temples';
import { notFound } from 'next/navigation';
import Link from 'next/link';


export default async function PadmanabhaswamyTemplePage() {
  const temple = await getTempleBySlug('padmanabhaswamy');
    ...
    <div className="container mx-auto px-4 py-12">
      <Card className="overflow-hidden shadow-lg mb-12">
        <div className="relative h-64 md:h-96">
          <Image
            src={temple.imageUrl || `https://picsum.photos/seed/padmanabhaswamy-temple/1200/400`}
            alt={temple.name}
            layout="fill"
            objectFit="cover"
            ...
            <CardContent className="space-y-4">
               {temple.architecture && (
                 <div>
                   ...
                   <ul className="list-disc list-inside text-muted-foreground text-sm space-y-1 pl-2">
                     ...
                     {/* Add more architectural details if available */}
                   </ul>
                 </div>
               )}
               {temple.deity && (
               ...
               {temple.governance && (
                 <div>
                   ...
                   <p className="text-muted-foreground text-sm">{temple.governance}</p>
                </div>
               )}
                 {temple.website && (
                 <div>
