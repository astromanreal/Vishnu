...
import { MapPin, Calendar, Landmark, Users, Building, Sparkles, Link as LinkIcon } from 'lucide-react';
import Link from 'next/link';
import { getTempleBySlug, type Temple } from '@/data/temples';
import { notFound } from 'next/navigation';

export default async function BirlaMandirPage() {
  const temple = await getTempleBySlug('birla-mandir');
    ...
    <div className="container mx-auto px-4 py-12">
      <Card className="overflow-hidden shadow-lg mb-12">
        <div className="relative h-64 md:h-96">
          <Image
            src={temple.imageUrl || `https://picsum.photos/seed/birla-mandir-delhi/1200/400`}
            alt={temple.name}
            layout="fill"
            objectFit="cover"
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
               {temple.architecture && (
                 <div>
                   ...
                   <ul className="list-disc list-inside text-muted-foreground text-sm space-y-1 pl-2">
                     {temple.architecture.type && <li><strong>Style:</strong> {temple.architecture.type}</li>}
                     {temple.architecture.creator && <li><strong>Builders:</strong> {temple.architecture.creator}</li>}
                     {temple.architecture.architect && <li><strong>Architect:</strong> {temple.architecture.architect}</li>}
               ...
               {temple.keyFeatures && temple.keyFeatures.length > 0 && (
                 <div>
                   <h3 className="font-semibold text-lg mb-2 text-primary/90 flex items-center gap-1"><Sparkles className="h-4 w-4" /> Key Features</h3>
                    <ul className="list-disc list-inside text-muted-foreground text-sm space-y-1 pl-2">
                       {temple.keyFeatures.map((feature, index) => (
