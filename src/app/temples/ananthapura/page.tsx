...
import { MapPin, Landmark, Users, Building, Globe, Droplet, Link as LinkIcon } from 'lucide-react';
import Link from 'next/link';
import { getTempleBySlug, type Temple } from '@/data/temples';
import { notFound } from 'next/navigation';

export default async function AnanthapuraLakeTemplePage() {
  const temple = await getTempleBySlug('ananthapura');
    ...
    <div className="container mx-auto px-4 py-12">
      <Card className="overflow-hidden shadow-lg mb-12">
        <div className="relative h-64 md:h-96">
          <Image
            src={temple.imageUrl || `https://picsum.photos/seed/ananthapura-lake/1200/400`}
            alt={temple.name}
            layout="fill"
            objectFit="cover"
            ...
            <CardContent className="space-y-4">
               {temple.architecture && (
                 <div>
                   ...
                   <ul className="list-disc list-inside text-muted-foreground text-sm space-y-1 pl-2">
                     <li><strong>Type:</strong> {temple.architecture.type}</li>
                     {/* Add more architectural details if available from temple.architecture */}
                   </ul>
                 </div>
               )}
               {temple.deity && (
               ...
               {temple.website && (
                 ...
                 {temple.keyFeatures && temple.keyFeatures.length > 0 && (
                   <div>
                     <h3 className="font-semibold text-lg mb-2 text-primary/90 flex items-center gap-1"><Droplet className="h-4 w-4" /> Key Features</h3>
                      <ul className="list-disc list-inside text-muted-foreground text-sm space-y-1 pl-2">
