...
import { MapPin, Landmark, Building, Users, Palette, Globe, Link as LinkIcon } from 'lucide-react';
import { getTempleBySlug, type Temple } from '@/data/temples';
import { notFound } from 'next/navigation';
import Link from 'next/link';

export default async function PundarikakshanTemplePage() {
  const temple = await getTempleBySlug('pundarikakshan');
    ...
    <div className="container mx-auto px-4 py-12">
      <Card className="overflow-hidden shadow-lg mb-12">
        <div className="relative h-64 md:h-96">
          <Image
            src={temple.imageUrl || `https://picsum.photos/seed/pundarikakshan-temple/1200/400`}
            alt={temple.name}
            layout="fill"
            objectFit="cover"
               ...
               {temple.architecture && (
                 <div>
                   ...
                   <ul className="list-disc list-inside text-muted-foreground text-sm space-y-1 pl-2">
                     ...
                     {temple.architecture.features && <li><strong>Key Feature:</strong> {temple.architecture.features}</li>}
                   </ul>
                 </div>
               )}
               {temple.deity && (
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
               ...
               {temple.architecture?.features?.includes('Swastika-shaped') && (
                 <div className="flex items-start">
                   <Palette className="h-4 w-4 mr-2 mt-0.5 text-accent flex-shrink-0" />
                   <span>Unique Swastika-shaped temple tank.</span>
                 </div>
               )}
