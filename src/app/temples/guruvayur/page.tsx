...
import { MapPin, Calendar, Landmark, Users, Building, Globe, Palette, Construction, Link as LinkIcon } from 'lucide-react';
import Link from 'next/link';
import { getTempleBySlug, type Temple } from '@/data/temples';
import { notFound } from 'next/navigation';

export default async function GuruvayurTemplePage() {
  const temple = await getTempleBySlug('guruvayur');
    ...
    <div className="container mx-auto px-4 py-12">
      <Card className="overflow-hidden shadow-lg mb-12">
        <div className="relative h-64 md:h-96">
          <Image
            src={temple.imageUrl || `https://picsum.photos/seed/guruvayur-temple/1200/400`}
            alt={temple.name}
            layout="fill"
            objectFit="cover"
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
               {temple.architecture && (
                 <div>
                   <h3 className="font-semibold text-lg mb-2 text-primary/90 flex items-center gap-1"><Palette className="h-4 w-4" /> Architecture & History</h3>
                   <ul className="list-disc list-inside text-muted-foreground text-sm space-y-1 pl-2">
                     ...
                     {temple.architecture.creator && <li><strong>Traditional Creators:</strong> {temple.architecture.creator}</li>}
                     {temple.architecture.completed && <li><strong>Period:</strong> {temple.architecture.completed}</li>}
                     {temple.architecture.specifications && <li><strong>Features:</strong> {temple.architecture.specifications}</li>}
                   </ul>
                 </div>
               )}
               {temple.deity && (
               ...
               {temple.governance && (
                 <div>
                   <h3 className="font-semibold text-lg mb-1 text-primary/90 flex items-center gap-1"><Construction className="h-4 w-4" /> Governance</h3>
                   <p className="text-muted-foreground">{temple.governance}</p>
                </div>
               )}
               {temple.website && (
                <div>
                   <h3 className="font-semibold text-lg mb-1 text-primary/90 flex items-center gap-1"><LinkIcon className="h-4 w-4"/> Official Website</h3>
               ...
               {temple.affiliation && (
                ...
                {temple.elevation && (
                  <div className="flex items-center">
                   ...
                   <span><strong>Elevation:</strong> {temple.elevation}</span>
                 </div>
                )}
