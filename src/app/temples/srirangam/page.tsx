...
import { MapPin, Calendar, Landmark, Users, Building, Globe, Star, Link as LinkIcon } from 'lucide-react';
import Link from 'next/link';
import { getTempleBySlug, type Temple } from '@/data/temples';
import { notFound } from 'next/navigation';

export default async function SrirangamTemplePage() {
  const temple = await getTempleBySlug('srirangam');
    ...
    <div className="container mx-auto px-4 py-12">
      <Card className="overflow-hidden shadow-lg mb-12">
        <div className="relative h-64 md:h-96">
          <Image
            src={temple.imageUrl || `https://picsum.photos/seed/srirangam-temple/1200/400`}
            alt={temple.name}
            layout="fill"
            objectFit="cover"
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
               {temple.architecture && (
                 <div>
                   ...
                   <ul className="list-disc list-inside text-muted-foreground text-sm space-y-1 pl-2">
                     ...
                     {temple.architecture.creator && <li><strong>Contributors:</strong> {temple.architecture.creator}</li>}
                     {temple.architecture.specifications && <li><strong>Key Features:</strong> {temple.architecture.specifications}</li>}
                     {temple.architecture.tower && <li><strong>Sanctum Tower:</strong> {temple.architecture.tower}</li>}
                   </ul>
                 </div>
               )}
               {temple.deity && (
               ...
               {temple.notes && (
                 <div>
                   <h3 className="font-semibold text-lg mb-1 text-primary/90">Significance</h3>
                    <p className="text-muted-foreground text-sm">{temple.notes}</p>
                </div>
               )}
               {temple.website && (
                <div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
               {temple.notes?.includes('Divya Desam #1') && (
                 <div className="flex items-center">
                   <Star className="h-4 w-4 mr-2 text-accent" />
                   <span><strong>Status:</strong> Divya Desam #1</span>
                 </div>
               )}
               {temple.affiliation && (
                 <div className="flex items-center">
